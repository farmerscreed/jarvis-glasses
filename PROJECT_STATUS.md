...
1. ~~**Glasses-button reactions**~~ **DONE + foreground service (2026-06-13):** protocol decoded + reactions live; the capture pipeline now lives in the shared `GlassesCaptureReactor`, and `ConnectedCompanionService` (foreground, `connectedDevice`) keeps it + the BLE link alive with the app backgrounded/killed (Settings → "Keep listening in the background"). **Live-press Look & Ask (double-click BACK) verified working** after fixing the BLE-connect-on-start gap. Remaining: background capture-while-killed director-verification; BLE auto-reconnect after transfer/idle drop. **Gotchas discovered (documented in `Glasses_Controls.md` §4):** glasses delete their files + emit a zero-inventory event after each sync (only an inventory *increase* = new capture); `BC 41` frames are command-ACK echoes, never events (parsing them as events causes a capture storm); audio records as `.opus` (route as `audio/ogg`); Wi-Fi-start ACK leaks SSID+passphrase.
