...
3. **Phase C — offline-first rebuild** (`docs/PRODUCTION_ROADMAP.md` §4). **C1+C2+C3 DONE & verified on device (2026-06-12):**
   - **C1** local-first memory core — every capture writes Room first then drains an outbox; `client_id` idempotency (server dedupes); `ConnectivityGovernor` (FULL/OFF_GRID) + "Cloud: …" chip. Verified: Wi-Fi off → queues locally → Wi-Fi on → drains, no dup.
   - **C2** background drain (WorkManager, survives app kill) + persisted session token. Verified: off-grid save → app process killed → Wi-Fi on → a fresh WorkManager-woken process drained to the server with no UI reopened.
   - **C3** on-device embeddings (bundled Universal Sentence Encoder via MediaPipe) for **offline semantic recall**. Verified: off-grid, "where did I leave my vehicle" (no shared words) returned "I parked on level 3…". Dual-embedding: local vector off-grid-only; Gemini 1536-dim canonical online.
   **C-next:** LEAN-tier RTT probe + degraded behaviour; Jarvis Lite on-device brain (phrase real offline answers); type-to-Jarvis modality fallback; deferred-vision/transcribe re-run for `needs_vision`/`needs_transcribe` rows; refresh-token rotation for long-lived background auth. Phase C is also the cost lever (§4.7).
