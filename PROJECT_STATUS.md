...
3. **Phase C — offline-first rebuild** (`docs/PRODUCTION_ROADMAP.md` §4). **C1 DONE & verified on device (2026-06-12):** local-first memory core — every capture writes Room first then drains an outbox to the cloud; `client_id` idempotency (server dedupes, retries never duplicate); `ConnectivityGovernor` (FULL/OFF_GRID) + "Cloud: online/OFF-GRID · N to sync" chip; vision/transcribe failures offline still save the file locally (tagged `needs_vision`/`needs_transcribe`). Verified: Wi-Fi off → note queues locally, not on server → Wi-Fi on → outbox drains, no dup, pending→0. **C-next:** background drain when app killed (WorkManager + Phase B foreground service); on-device embeddings for semantic offline recall; LEAN-tier RTT probe; Jarvis Lite on-device brain; type-to-Jarvis modality fallback. This is also the cost lever (§4.7).
